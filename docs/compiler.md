...
*   **Raw Pointers & Mapped I/O:** Variables can hold raw integer addresses (`ptr = 65000`). Dereferencing is done via explicit assignment to a bracketed pointer: `[ptr] = sum;` writes the value of `sum` directly to physical memory.
*   **The `[view]` Lens & Direct Destructuring:** Because memory is just raw bits, types are merely "Lenses" that instruct the compiler how to interpret, slice, or offset data. Lenses can be used as direct output targets. For instance, a function return tuple can destructure directly into a memory address, bypassing local variables entirely:
  ```rs
  // Destructures the return of `loop` into a local `sum` and directly into memory at `ptr`
  sum, [ptr] = loop(1, i, sum, ptr);
  ```
*   **Splatting:** Data arrays and tuples are flat. Destructuring extracts data natively via sequential hardware registers: `x, y, z = bytes`.
...
*   **Functions as value (TODO):** Passed by pointer to logic location. With strict VMG rules in place, this is statically resolvable at compile time.
...
*   **Function Pointers:** While the syntax allows passing functions as values, true dynamic dispatch over raw memory addresses is heavily restricted to prevent opaque side-effects. Currently, function resolution is strictly evaluated at compile-time via the static registry.
...
*   **Comptime Resolution Hierarchy:** 
    1.  *Static:* Purely constant expressions (`1 + 2`) are evaluated and folded during compilation via JIT CPU emulation.
...
*   **With Dot (Static / Namespace Registry):** The `.` prefix explicitly tells the compiler: *"Bypass the dynamic stack and anchor this construct statically to the `{}` namespace I am currently inside."*
    ...
    *   `.@expr`: Emits a macro directly into the compiler's static Macro Registry. (technically macros don't actually exist outside comptime)
...
## 11. The VM Sandbox & System Interfaces
Because the language relies on bare-metal memory structures, execution happens within a sandboxed virtual environment (currently running via a custom Zig VM). 

Hardware interactions, such as I/O, are mediated strictly through the RISC-V `ecall` instruction using a tailored application binary interface (ABI). The ABI maps the `a7` (`x17`) register to specific system calls:

*   **0 (HALT):** Safely terminates VM execution.
*   **1 (PRINT_INT):** Prints the integer currently held in register `a0`.
*   **2 (WRITE):** Writes a byte stream to `stdout`. Reads `a2` bytes starting from the physical memory address held in `a1`.
*   **3 (READ):** Reads a byte stream from `stdin`. Writes `a2` bytes into the physical memory address held in `a1`. The actual number of bytes read is returned into `a0`.

This sandboxed approach guarantees that despite the language having unfettered capability to write memory directly (e.g., `[ptr] = 65000`), it is physically prevented from violating the host system's bounds.
